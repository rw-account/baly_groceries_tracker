<div dir="rtl">

# Baly Groceries Tracker
<br>

[![License](https://img.shields.io/badge/license-MIT-4CAF50.svg)](LICENSE)
[![Platform](https://img.shields.io/badge/platform-Android-3DDC84?logo=android&logoColor=white)](android/)
[![Flutter](https://img.shields.io/badge/flutter-3.x-02569B?logo=flutter&logoColor=white)](https://flutter.dev)
[![Dart](https://img.shields.io/badge/dart-3.x-0175C2?logo=dart&logoColor=white)](https://dart.dev)
[![State](https://img.shields.io/badge/state-Riverpod-00599C)](https://riverpod.dev)
[![Database](https://img.shields.io/badge/database-SQLite-003B57?logo=sqlite&logoColor=white)](https://www.sqlite.org)
![100% Local First](https://img.shields.io/badge/privacy-100%25%20Local--First-2E7D32?logo=shield&logoColor=white)
![No Ads](https://img.shields.io/badge/ads-Free%20%26%20No%20Ads-FF9800)
[![Languages](https://img.shields.io/badge/languages-Arabic%20%7C%20English-7952B3)](lib/l10n/)

تطبيق مجاني ومفتوح المصدر يمنحك نظرة واحدة سريعة لتعرف بدقة الكميات المتبقية من البيض، الدقيق، الأرز، وكافة احتياجاتك، لتكتشف فوراً ما أوشك على النفاد وما يزال متوفرًا.

يساعدك التطبيق على ترتيب أولوياتك بذكاء، مما يمنحك تحكماً كاملاً في ميزانية المنزل. بفضل التنبيهات الاستباقية للمواد التي أوشكت على الانتهاء، ستتمكن من توجيه مصاريفك نحو الأساسيات أولاً **قبل استهلاك الميزانية في كماليات غير ضرورية**. هذا التنظيم الذكي يحميك من الأزمات المالية، ويزيل عنك قلق نقص الاحتياجات، لتستمتع براحة البال وإدارة منزلية أكثر استقراراً، مثالي للعائلات وكل من يرغب في حياة افضل و اكثر تنظيما.

## الميزات الرئيسية

### شاشة "النفاد"

تعرض جميع المواد التي أوشكت على النفاد، مرتبة حسب الأولوية مع مؤشرات بصرية واضحة، لتعرف فورًا ما يجب شراؤه أولًا.

### تنبيهات ذكية

احصل على تنبيهات مسبقة قبل نفاد احتياجات المنزل بوقت كافٍ، لتتمكن من التخطيط لشرائها مسبقًا. كما يتاح لك ضبط توقيت هذه التنبيهات أو إيقاف تفعيلها لأي مادة في أي وقت.

### قائمة تسوق مدمجة

أضف المواد التي أوشكت على النفاد، إلى جانب أي مواد أخرى، لتشتري ما تحتاجه فقط وتتجنب المشتريات غير الضرورية. كما يمكنك مشاركة القائمة بسهولة عبر واتساب مع أي شخص يتولى التسوق.

### مشاركة المخزون

أنشئ ملخصًا يوضح المواد المتوفرة والمدة المتبقية لكل منها، ثم شاركه مباشرة عبر واتساب مع أفراد العائلة.

### سجل التغييرات

راجع جميع التعديلات التي أُجريت على أي عنصر، واعرف متى تم تعديلها، مع إمكانية استعادة أي حالة سابقة بسهولة.

### خصوصية كاملة

تُحفظ جميع بياناتك محليًا على جهازك، ويعمل التطبيق بالكامل دون اتصال بالإنترنت، مع إمكانية إنشاء نسخ احتياطية واستعادتها عند الحاجة.

### بدون إعلانات

استمتع بتجربة استخدام نظيفة وخالية تمامًا من الإعلانات.

### يدعم العربية والإنجليزية

استخدم التطبيق بلغتك المفضلة مع دعم كامل للغتين العربية والإنجليزية.

## لقطات الشاشة

<br>

<div align="center">
  <table>
    <tr>
      <td align="center"><b>الرئيسية</b></td>
      <td align="center"><b>شاشة النفاد</b></td>
    </tr>
    <tr>
      <td align="center">
        <img src="assets/screenshots/ar/home_ar.png" alt="Arabic home screen" width="260">
      </td>
      <td align="center">
        <img src="assets/screenshots/ar/expiry_ar.png" alt="Arabic expiry screen" width="260">
      </td>
    </tr>
    <tr>
      <td align="center"><b>قائمة الشراء</b></td>
      <td align="center"><b>تعديل المادة</b></td>
    </tr>
    <tr>
      <td align="center">
        <img src="assets/screenshots/ar/shopping_list_ar.png" alt="Arabic shopping list screen" width="260">
      </td>
      <td align="center">
        <img src="assets/screenshots/ar/edit_item_ar.png" alt="Arabic edit item screen" width="260">
      </td>
    </tr>
    <tr>
      <td align="center"><b>مشاركة تفاصيل المواد</b></td>
      <td align="center"><b>مشاركة قائمة الشراء</b></td>
    </tr>
    <tr>
      <td align="center">
        <img src="assets/screenshots/ar/share_item_details_ar.png" alt="Arabic share item details screen" width="260">
      </td>
      <td align="center">
        <img src="assets/screenshots/ar/share_shopping_list_ar.png" alt="Arabic share shopping list screen" width="260">
      </td>
    </tr>
  </table>
</div>

## روابط إضافية

<ul dir="rtl">

<br>

<li>

[خطوات البناء وتجميع التطبيق من المصدر (Building from Source)](README.md#building-from-source)
</li>

<br>

<li>

[دليل المساهمة في المشروع (Contributing)](README.md#contributing)
</li>

<br>

<li>

[الرخصة والعلامة التجارية (License & Trademark)](README.md#license--trademark)
</li>

</ul>

</div>
